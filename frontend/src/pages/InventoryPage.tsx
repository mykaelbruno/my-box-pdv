...
import { Badge, Button, Field, Modal, NumericInput, PageHeader, Progress, Section, Toast, type ToastTone } from '../components/Ui'
...
function ProductEditor({ draft, mode, onChange, onScan }: ProductEditorProps) {
  const baseUnitDecimalScale = draft.unidade === 'KG' ? 2 : 0

  const changeBaseUnit = (unidade: Product['unidade']) => {
    onChange({
      ...draft,
      unidade,
      minimo: unidade === 'UN' ? Math.round(draft.minimo) : draft.minimo,
      presentations: draft.presentations.map((presentation) => ({
        ...presentation,
        fatorBase: unidade === 'UN' ? Math.max(1, Math.round(presentation.fatorBase)) : presentation.fatorBase,
      })),
    })
  }
  ...
  return (
    <>
      <div className="form-grid product-editor__base">
        ...
        <Field label="Unidade base"><select value={draft.unidade} onChange={(event) => changeBaseUnit(event.target.value as Product['unidade'])}><option value="UN">Unidade (UN)</option><option value="KG">Quilograma (KG)</option></select></Field>
        <Field label="Estoque mínimo"><NumericInput decimalScale={baseUnitDecimalScale} min={0} value={draft.minimo} onValueChange={(minimo) => onChange({ ...draft, minimo })} /></Field>
        <Field label="Custo da unidade base"><NumericInput min={0} value={draft.custo} onValueChange={(custo) => onChange({ ...draft, custo })} /></Field>
          ...
          {draft.presentations.map((presentation, index) => (
            <article className="presentation-editor" key={presentation.id}>
              ...
              <div className="form-grid">
                ...
                <Field label="Converte para"><NumericInput decimalScale={baseUnitDecimalScale} min={draft.unidade === 'KG' ? 0.01 : 1} value={presentation.fatorBase} onValueChange={(fatorBase) => updatePresentation(index, { fatorBase })} /></Field>
                <Field label="Preço de venda"><NumericInput min={0.01} value={presentation.preco} onValueChange={(preco) => updatePresentation(index, { preco })} /></Field>
