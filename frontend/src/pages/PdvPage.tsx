...
type ModalName = 'product' | 'manual' | 'adjust' | 'payment' | 'cancel' | null
...
export function PdvPage() {
  ...
  const addCart = () => {
    const id = Math.max(...carts.map((cart) => cart.id), 0) + 1
    const code = `#${1047 + id}`
    setCarts((current) => [...current, { id, codigo: code, horario: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }), operador: 'Coutinho', items: [] }])
    setActiveId(id)
    setToast({ message: `Venda ${code} iniciada`, tone: 'success' })
  }
  ...
  return (
    <div className="pdv-page">
      <header className="pdv-header">
        ...
        <Button variant="primary" icon={<Plus size={18} />} onClick={addCart}>Nova venda</Button>
      </header>
      ...
      <div className="pdv-layout">
        <aside className="cart-rail">
          ...
          <div className="cart-tabs">
            {carts.map((cart) => (
              <button key={cart.id} className={cart.id === activeId ? 'active' : ''} onClick={() => setActiveId(cart.id)}>
                <div><strong>{cart.codigo}</strong><span>{cart.horario}</span></div>
                <small>{cart.items.length ? `${cart.items.length} itens` : 'Carrinho vazio'}</small>
        ...
        <aside className="sale-summary">
          ...
          <div className="summary-actions">
            ...
            <Button variant="primary" icon={<Check size={18} />} onClick={() => setModal('payment')} disabled={!activeCart.items.length}>Finalizar <span className="summary-actions__value">{money(total)}</span></Button>
          </div>
        </aside>
      </div>
      ...
      {modal === 'payment' && (
        <Modal title="Finalizar venda" description={`${activeCart.codigo} · ${activeCart.items.length} itens`} size="large" onClose={() => setModal(null)} footer={<><Button variant="quiet" onClick={() => setModal(null)}>Voltar</Button><Button variant="primary" icon={<Check size={18} />} onClick={finishSale}>Confirmar pagamento</Button></>}>
          ...
          <div className="payment-methods">
            {([{ name: 'Dinheiro', icon: Banknote }, { name: 'Pix', icon: QrCode }, { name: 'Cartão', icon: CreditCard }, { name: 'Fiado', icon: UserRound }] as { name: PaymentMethod; icon: typeof Banknote }[]).map(({ name, icon: Icon }) => <button key={name} className={method === name ? 'active' : ''} onClick={() => { setMethod(name); setSplitPayment(false) }}><Icon size={21} /><span>{name}</span>{method === name && <Check size={15} />}</button>)}
          </div>
